specs:
  ...
  - id: agents_agent-files_spec-curator_wired-opencode
    description: The spec-curator agent file is made available inside project containers by
      bind-mounting agents/agent-files onto /home/agent/.config/opencode/agents, opencode's global
      markdown agents directory.
    motivation: A single directory mount wires every agent file under agents/agent-files into the
      container without per-agent entries in the container opencode config.
    acceptance_criteria:
      - docker/docker-compose.yml contains a bind mount from ../agents/agent-files to
